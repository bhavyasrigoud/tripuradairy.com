...
function OrderPage() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", preferredDate: tomorrowISO(), deliveryTime: "" });
  ...
  const unitPrice = product ? product.sizePrices[size] ?? 0 : 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    openWhatsApp(
      fullOrderMessage({
        ...
        count: product ? count : undefined,
        amount: product ? formatINR(totalAmount) : undefined,
        name: form.name,
        phone: form.phone,
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-14">
      <div className="text-center">
        <h1 className="font-serif text-4xl font-bold text-foreground">Place Your Order</h1>
      ...
      >
        {product ? (
          <div className="space-y-5 rounded-xl border border-border/60 bg-secondary/30 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gold-deep">{product.price}</p>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-dashed border-border/60 bg-secondary/30 p-6 text-center">
            <p className="text-muted-foreground">No product selected yet.</p>
            <Button asChild variant="outline" className="mt-4 border-gold-deep text-gold-deep hover:bg-accent">
